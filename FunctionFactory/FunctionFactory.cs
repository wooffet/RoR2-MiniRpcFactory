...
namespace MiniRpcFactory.FunctionFactory
{
    internal sealed class FunctionFactory
    {
        private static Lazy<FunctionFactory> _commandFactory { get; set; }
        private static FunctionFactory _instance { get { return _commandFactory.Value; } }

        internal static FunctionFactory CreateFactoryInstance()
        {
            if (!_commandFactory.IsValueCreated)
            {
                _commandFactory = new Lazy<FunctionFactory>(() => new FunctionFactory());
            }
        ...
        internal delegate FunctionCommand<RequestType, ResponseType> ConstructorDelegate<RequestType, ResponseType>(params object[] args);
